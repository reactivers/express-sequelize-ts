...
export const getUserByPassword: (
  params: ILoginInput,
) => Promise<IUser> = async ({ username, password }) => {
...
export const getUserByUsername: (username: string) => Promise<IUser> = async (
  username: string,
) => {
  const obj = await Users.findOne({
    where: {
...
export const findUserByToken: (token: string) => Promise<IUser> = async (
  token: string,
) => {
...
export const getUserByRequest: (request: Request) => Promise<IUser> = async (
  request,
) => {
